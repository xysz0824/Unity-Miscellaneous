...
namespace Coffee.UIParticleExtensions
{
    ...
    internal static class MeshPool
    {
        ...
        {
            s_PoolCount = 32;
            s_Pool = new Mesh[s_PoolCount];
            for (var i = 0; i < s_PoolCount; i++)
            {
                var m = new Mesh();
                m.MarkDynamic();
                s_Pool[i] = m;
        ...
        static bool Contains(Mesh mesh)
        {
            for (int i = 0; i < s_PoolCount; ++i)
            {
                if (object.ReferenceEquals(s_Pool[i], mesh))
                    return true;
            }
            return false;
        }
        ...
        public static void Return(Mesh mesh)
        {
            if (!mesh) return;
            mesh.Clear(false);
            if (Contains(mesh)) return;
            if (s_Pool.Length <= s_PoolCount)
            {
                Array.Resize(ref s_Pool, s_Pool.Length * 2);
            }
            s_Pool[s_PoolCount++] = mesh;
        }
    }
